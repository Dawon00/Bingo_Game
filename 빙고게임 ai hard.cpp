...
int main()
{
	...
	while (true)
	{
		...
		switch (iAIMode)
		{
			...
			/*
			AI Easy 모드는 현재 AI의 숫자 목록중*로 바뀌지 않은 숫자목록을 만들어서
			그 목록중 하나를 선택하게 한다.(랜덤하게)
			*/
		case AM_EASY:
			//선택안된 숫자목록을 만들어준다.
			//선택안된 숫자 개수는 목록을 만들때 카운트해준다.
			iNoneSelectCount = 0;
			for (int i = 0; i < 25; ++i)
			{
				//현재 숫자가 *이 아닐경우
				if (iAINumber[i] != INT_MAX)
				{
					...
					//선택안된 목록 개수에 추가할때마다 개수를 1씩 증가시켜서
					//총 선택안된 개수를 구해준다.
			...
			break;
		case AM_HARD:
			// 하드모드는 현재 숫자중 빙고줄 완성 가능성이 가장 높은 
			//줄을 찾아서 그 줄에 있는 숫자중 하나를 *로 만들어준다.
			int iLine=0;
			int iStarCount=0;
			int iSaveCount =0;

			//가로 라인중 가장 *이 많은 줄을 찾아낸다
			for (int i = 0; i < 5; ++i)
			{
				iStarCount = 0;
				for (int j = 0; j < 5; ++j)
				...
				//별이 5개 미만이어야 빙고 줄이 아니고 기존에 가장
				//많은 라인의 별보다 커야 가장 별이 많은 라인이므로
				//라인을 교체해주고 저장된 별 수를 교체한다.
				if (iStarCount < 5 && iSaveCount < iStarCount)
				{
					//여기는 가로 라인중 가장 별이 많은 라인을 체크하는곳
			...
			//모든 라인을 조사했으면 iLine에 가능성이 가장 높은
			//줄 번호가 저장되었다.
			//그 줄에 있는 *이 아닌 숫자중 하나를 선택하게한다.
			//가로줄일 경우
			if (iLine <= LN_H5)
			{
		...
		//빙고 줄수를 체크하는 것은 매번 숫자를 입력할때마다 처음부터
		...
		for (int i = 0; i < 5; ++i)
		{
			...
			// iStart1 변수에 들어가게 된다. iStart1이 값이 5이면 한줄이
